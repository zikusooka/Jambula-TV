...
$VLC_CMD $VLC_OPTIONS v4l2://$VDEV --input-slave=alsa://$ADEV --alsa-samplerate=$SAMPLE_RATE --v4l2-standard=$V4L2_STANDARD --v4l2-chroma=$CHROMA --v4l2-input=0 --v4l2-audio-input=0 --v4l2-width=$WIDTH --v4l2-height=$HEIGHT --v4l2-tuner=0 --v4l2-tuner-frequency=$FREQ --v4l2-tuner-audio-mode=1 --logo-file $LOGOFILE --sout-mux-caching=$MUX_CACHE --sout '#transcode{vcodec='$VCODEC',sfilter=logo,acodec='$ACODEC',vb='$VB',ab='$AB',venc=ffmpeg{keyint=80,hurry-up,vt=800000},audio-sync,aenc=ffmpeg,samplerate=44100,deinterlace=blend}:standard{access=livehttp{index=/usr/share/JambulaTV/html/android/iphonestream.m3u8,index-url=http://192.168.0.250/android/iphonestream.ts},mux='$MUX'{'$STREAM_TITLE'},dst=/usr/share/JambulaTV/html/android/iphonestream.ts}"}' 

exit

std{
access=livehttp{index=/usr/share/JambulaTV/html/android/iphonestream.m3u8,index-url=http://192.168.0.250/android/iphonestream.ts},mux=ts{use-key-frames},dst=/usr/share/JambulaTV/html/android/iphonestream.ts}
